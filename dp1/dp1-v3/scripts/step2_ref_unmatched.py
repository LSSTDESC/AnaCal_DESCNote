...
def split_matched(*, ref_path, our_ra, our_dec,
    ...
    if np.isfinite(ref_imag_max):
        imag = flux_to_mag(iflux, mag_zero=mag_zero)[0]
...
def main():
    ...
    our_iflux = np.asarray(anacal["lsst_i_flux_gauss2"], dtype=np.float64)
    our_imag = flux_to_mag(our_iflux, mag_zero=args.mag_zero)[0]
