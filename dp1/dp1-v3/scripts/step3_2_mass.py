...
def main():
    ...
    # global_R = R_shape + R_detect (mean over selected sources) + R_sel
    # (scalar selection-response term).  R_sel is only added when the
    # caller passed --delta-gamma > 0; otherwise ctx["r_sel"] is None and
    # global_R falls back to shape+detect only, matching pre-R_sel runs.
    r_shape_detect = float(np.mean(res))
    r_sel = ctx.get("r_sel")
    global_R = r_shape_detect + (r_sel or 0.0)
    if r_sel is None:
        print(f"# global R = {global_R:.4f}  (shape+detect only; "
              f"pass --delta-gamma 0.01 to include R_sel)", file=sys.stderr)
    else:
        print(f"# global R = {global_R:.4f}  "
              f"(shape+detect={r_shape_detect:.4f}, R_sel={r_sel:+.4f})",
              file=sys.stderr)
    galcat = GCData()
    galcat["ra"] = table["ra"]
